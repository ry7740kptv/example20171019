...
void Speaker_Reset()
{
  ...
  SetEPDblBuffCount(ENDP1, EP_DBUF_OUT, 320);
  ...
  SetEPDblBuffCount(ENDP2, EP_DBUF_IN, 3);
...
/*******************************************************************************
* Function Name  : Speaker_SetInterface
* Description    : Update the device interface in use.
* Input          : None.
* Output         : None.
* Return         : None.
*******************************************************************************/
...
{
	if(pInformation->USBwIndexs.w == 0x0100){ /* Interface 1, Standard AS Interface.*/
		if(pInformation->USBwValues.w == 0x0100) /* Operational */
			flag_playing = 1;
		else /* Zero Bandwith */
			flag_playing = 0;
	}
...
{	
	if(Length == 0){
		...
		if((pInformation->USBbmRequestType & 0x03) == 1){
			if(pInformation->USBwValues.bw.bb1 == 0x01) /*MUTE_CONTROL*/
				return((uint8_t*)(&MUTE_DATA));
		}else{
			if(pInformation->USBwValues.bw.bb1 == 0x01) /* SAMPLING_FREQ_CONTROL */
